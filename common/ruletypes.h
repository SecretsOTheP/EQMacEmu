...
RULE_INT(Quarm, QuakeEndTimeDuration, 84600, "24 Hours")
...
RULE_INT(Quarm, TestPopulationOffset, 0, "Test population offset for queue testing")
RULE_INT(Quarm, IPDatabaseSyncInterval, 300, "Interval in seconds between database synchronization of IP reservation data")
RULE_INT(Quarm, MaxPlayersOnline, 1200, "Maximum number of players allowed online before queue is activated")
RULE_BOOL(Quarm, EnableQueue, true, "Enable the login queue system when server population is at capacity")
RULE_BOOL(Quarm, QueueBypassGMLevel, true, "Allow GMs (status >= 80) to bypass the queue")
RULE_BOOL(Quarm, EnableQueueLogging, true, "Enable detailed logging for queue system operations")
RULE_BOOL(Quarm, FreezeQueue, false, "Freeze queue advancement - players remain at current positions")
RULE_BOOL(Quarm, EnableQueuePersistence, true, "Enable saving queue state to database for crash recovery")
RULE_INT(Quarm, DefaultGracePeriod, 60, "Default grace period in seconds for IP reservations when not specified")
RULE_INT(Quarm, IPCleanupInterval, 5, "Interval in seconds between IP reservation cleanup cycles")
RULE_CATEGORY_END()
