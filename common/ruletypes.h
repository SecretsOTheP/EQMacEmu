...
RULE_BOOL(Quarm, EnableLoginQueue, false, "Enable queuing of accounts at server select instead of rejecting them once the player cap is reached.")
RULE_INT(Quarm, QueueEntryTimeoutS, 60, "Seconds a queue entry survives without the login server re-polling for that account.")
RULE_INT(Quarm, QueueSlotHoldS, 300, "Keep population slot for account after its last connection ends. Allows for bypassing of queue within timeframe for accidental diconnect / linkdead / etc.")
