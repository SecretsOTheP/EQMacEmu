...
void command_resetaa(Client* c, const Seperator *sep){
	if (c->GetTarget() != 0 && c->GetTarget()->IsClient()){
		...
		c->Message(Chat::Default, "Usage: Target a client and use #resetaa to reset the AA data in their Profile.");
}
...
void command_giveplayerfaction(Client *c, const Seperator *sep)
{
	...
	{
		c->Message(Chat::Default, "Usage: #giveplayerfaction [factionid] [value]");
		return;
	}
	...
	c->Message(Chat::Default, "%s was given %i points with %s.", t->GetName(), value, name);
}
...
void command_viewplayerfaction(Client *c, const Seperator *sep)
{
	...
	{
		c->Message(Chat::Default, "Usage: #viewplayerfaction [factionid 1-5999]");
		return;
	}
	...
	c->Message(Chat::Default, "%s has %d personal and %d modified faction with '%s' (%d)  personal cap: %d to %d; modified cap: %d to %d",
		t->GetName(), personal, modified, name, factionid, min_cap, max_cap, modified_min_cap, modified_max_cap);
	if (illusioned != modified)
		c->Message(Chat::Default, "Illusion is active and fooling this faction.  Illusioned faction value is %d", illusioned);

	if (npc != nullptr)
	{
		modified = t->GetFactionValue(npc);
		c->Message(Chat::Default, "Effective faction for '%s' is %d and includes extra modifiers such as invis and aggro", npc->GetName(), modified);
	}
}
...
void command_testspawn(Client *c, const Seperator *sep){
	if (sep->IsNumber(1)) {
		...
		if (sep->IsHexNumber(2)) {
			if (strlen(sep->arg[2]) >= 3) // 0x00, 1 byte
				...
				*((uint32*)&(((uint8*)&ns->spawn)[atoi(sep->arg[1])])) = hextoi(sep->arg[2]);
				...
				c->Message(Chat::Default, "Error: unexpected hex string length");
		}
		else {
		...
		c->Message(Chat::Default, "Usage: #testspawn [memloc] [value] - spawns a NPC for you only, with the specified values set in the spawn struct");
}

void command_wc(Client *c, const Seperator *sep){
	if (sep->argnum < 2)
	{
		c->Message(Chat::Default, "Usage: #wc slot material [color] [unknown06]");
	}
...
void command_setanim(Client *c, const Seperator *sep){
	if (c->GetTarget() && sep->IsNumber(1)) {
		...
		if (num < 0 || num >= _eaMaxAppearance) {
			c->Message(Chat::Default, "Invalid animation number, between 0 and %d", _eaMaxAppearance - 1);
		}
		c->GetTarget()->SetAppearance(EmuAppearance(num));
	}
	else
		c->Message(Chat::Default, "Usage: #setanim [animnum]");
}
...
void command_chat(Client *c, const Seperator *sep){
	if (sep->arg[2][0] == 0)
		c->Message(Chat::Default, "Usage: #chat [channum] [message]");
	else
...
void command_npcloot(Client *c, const Seperator *sep){
	...
	// These 2 types are *BAD* for the next few commands
		...
		c->Message(Chat::Default, "Error: Invalid target type, try a NPC =).");
	// #npcloot add
	else if (strcasecmp(sep->arg[1], "add") == 0)
	{
		...
		if (c->GetTarget()->IsNPC() && sep->IsNumber(2))
		{
			...
			if (dbitem)
			{
				...
				c->Message(Chat::Default, "Added item(%i) to %s's loot.", item, c->GetTarget()->GetCleanName());
			}
			else
				c->Message(Chat::Default, "Error: #npcloot add: Item(%i) does not exist!", item);
		}
		else if (!sep->IsNumber(2))
			c->Message(Chat::Default, "Error: #npcloot add: Itemid must be a number.");
		else
			c->Message(Chat::Default, "Error: #npcloot add: This is not a valid target.");
	}
	// #npcloot remove
	else if (strcasecmp(sep->arg[1], "remove") == 0)
	{
		...
		if (strcasecmp(sep->arg[2], "all") == 0)
		{
			...
			c->Message(Chat::Default, "Removed all loot from %s.", c->GetTarget()->GetCleanName());
		}
		...
		else
		{
			if (c->GetTarget()->IsNPC() && sep->IsNumber(2))
			{
				...
				if (npc)
				{
					...
					c->Message(Chat::Default, "Removed item(%i) from %s's loot.", item, npc->GetCleanName());
				}
			}
			else if (!sep->IsNumber(2))
				c->Message(Chat::Default, "Error: #npcloot remove: Item must be a number.");
			else
				c->Message(Chat::Default, "Error: #npcloot remove: This is not a valid target.");
		}
	}
	// #npcloot money
	else if (strcasecmp(sep->arg[1], "money") == 0)
	{
		if (c->GetTarget()->IsNPC() && sep->IsNumber(2) && sep->IsNumber(3) && sep->IsNumber(4) && sep->IsNumber(5))
		{
			if ((atoi(sep->arg[2]) < 34465 && atoi(sep->arg[2]) >= 0) && (atoi(sep->arg[3]) < 34465 && atoi(sep->arg[3]) >= 0) && (atoi(sep->arg[4]) < 34465 && atoi(sep->arg[4]) >= 0) && (atoi(sep->arg[5]) < 34465 && atoi(sep->arg[5]) >= 0))
			{
				...
				c->Message(Chat::Default, "Set %i Platinum, %i Gold, %i Silver, and %i Copper as %s's money.", atoi(sep->arg[2]), atoi(sep->arg[3]), atoi(sep->arg[4]), atoi(sep->arg[5]), c->GetTarget()->GetName());
			}
			else
				c->Message(Chat::Default, "Error: #npcloot money: Values must be between 0-34465.");
		}
		else
			c->Message(Chat::Default, "Usage: #npcloot money platinum gold silver copper");
	}
	else
		c->Message(Chat::Default, "Usage: #npcloot [show/money/add/remove] [itemid/all/money: pp gp sp cp] [quantity] [quest]");
}
...
void command_gm(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] != 0) {
		t->SetGM(state);
		c->Message(Chat::Default, "%s is %s a GM.", t->GetName(), state ? "now" : "no longer");
	}
	else
		c->Message(Chat::Default, "Usage: #gm [on/off]");
}
...
void command_zone(Client *c, const Seperator *sep)
{
	...
	if (!arguments) {
		c->Message(Chat::Default, "Usage: #zone [Zone ID|Zone Short Name] [X] [Y] [Z]");
		return;
	}
...
void command_peqzone(Client *c, const Seperator *sep){
	...
	if (c->GetHPRatio() < 75.0f) {
		c->Message(Chat::Default, "You cannot use this command with less than 75 percent health.");
		return;
	}
	...
	if (
		...
		c->Message(Chat::Default, "You cannot use this command in your current state. Settle down and wait.");
		return;
	}
	...
	{
		c->Message(Chat::Default, "Usage: #peqzone [zonename]");
		c->Message(Chat::Default, "Optional Usage: #peqzone [zoneid]");
		return;
	}
	else {
		...
		destzone = database.GetPEQZone(zoneid);
		if (zoneid == 0) {
			c->Message(Chat::Default, "Unable to locate zone '%s'", sep->arg[1]);
			return;
		}
		if (destzone == 0){
			c->Message(Chat::Red, "You cannot use this command to enter that zone!");
...
void command_movechar(Client *c, const Seperator *sep){
	if (sep->arg[1][0] == 0 || sep->arg[2][0] == 0)
		c->Message(Chat::Default, "Usage: #movechar [charactername] [zonename]/bind");
		...
		c->Message(Chat::Default, "Invalid zone name");
	else
	{
		...
		if (tmp)
		{
			if (c->Admin() >= commandMovecharSelfOnly || tmp == c->AccountID())
			{
				if (strcasecmp(sep->arg[2], "bind") == 0)
				{
					...
					if(zone_id == 0)
						c->Message(Chat::Default, "Character Move Failed!");
					else
						c->Message(Chat::Default, "%s has been moved to %s.", (char*)sep->arg[1], database.GetZoneName(zone_id));
				}
				else if (!database.MoveCharacterToZone((char*)sep->arg[1], (char*)sep->arg[2]))
					c->Message(Chat::Default, "Character Move Failed!");
				else
					c->Message(Chat::Default, "%s has been moved to %s.", (char*)sep->arg[1], (char*)sep->arg[2]);
			}
			else
				c->Message(Chat::Red, "You cannot move characters that are not on your account.");
		}
		else
			c->Message(Chat::Default, "Character Does Not Exist");
	}
}
...
void command_date(Client *c, const Seperator *sep)
{
	...
	if (
		...
		c->Message(Chat::Default, "Usage: #date [Year] [Month] [Day] [Hour] [Minute]");
		c->Message(Chat::Default, "Hour and Minute are optional");
		return;
	}
	...
	c->Message(Chat::Default, fmt::format("Setting world time to {}-{}-{} {}:{}...", year, month, day, hour, minute).c_str());
...
void command_timezone(Client *c, const Seperator *sep)
{
	...
	if (arguments < 1 || sep->IsNumber(1)) {
		c->Message(Chat::Default, "Usage: #timezone HH [MM]");
		c->Message(Chat::Default, fmt::format("Current timezone is: {}h {}m", zone->zone_time.getEQTimeZoneHr(), zone->zone_time.getEQTimeZoneMin()).c_str());
		return;
	}
	...
	c->Message(Chat::Default, fmt::format("Setting timezone to {} h {} m", hours, minutes).c_str());
...
{
	c->Message(Chat::Default, "Updating Time/Date for all clients in zone...");
	auto outapp = new EQApplicationPacket(OP_TimeOfDay, sizeof(TimeOfDay_Struct));
	...
	if (sep->arg[3][0] == 0)
		...
		if (strcasecmp(sep->arg[1], "zone") == 0){
			...
			if (strstr(sep->arg[3], "^") == 0)
				entity_list.Message(Chat::Default, atoi(sep->arg[2]), sep->argplus[3]);
			else{
				for (newmessage = strtok((char*)sep->arg[3], "^"); newmessage != nullptr; newmessage = strtok(nullptr, "^"))
					entity_list.Message(Chat::Default, atoi(sep->arg[2]), newmessage);
			}
		}
...
void command_zclip(Client *c, const Seperator *sep){
	...
	if (sep->arg[2][0] == 0)
		c->Message(Chat::Default, "Usage: #zclip <min clip> <max clip>");
	else if (atoi(sep->arg[1]) <= 0)
		c->Message(Chat::Default, "ERROR: Min clip can not be zero or less!");
	else if (atoi(sep->arg[2]) <= 0)
		c->Message(Chat::Default, "ERROR: Max clip can not be zero or less!");
	else if (atoi(sep->arg[1])>atoi(sep->arg[2]))
		c->Message(Chat::Default, "ERROR: Min clip is greater than max clip!");
	else {
...
void command_npccast(Client *c, const Seperator *sep){
	if (c->GetTarget() && c->GetTarget()->IsNPC() && !sep->IsNumber(1) && sep->arg[1] != 0 && sep->IsNumber(2)) {
		...
		if (spelltar)
			...
			c->Message(Chat::Default, "Error: %s not found", sep->arg[1]);
	}
		...
		if (spelltar)
			...
			c->Message(Chat::Default, "Error: target ID %i not found", atoi(sep->arg[1]));
	}
	else
		c->Message(Chat::Default, "Usage: (needs NPC targeted) #npccast targetname/entityid spellid");
}
...
void command_permarace(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] == 0) {
		c->Message(Chat::Default, "Usage: #permarace <racenum>");
		c->Message(Chat::Default, "NOTE: Not all models are global. If a model is not global, it will appear as a human on character select and in zones without the model.");
	}
	else if (!t->IsClient())
		c->Message(Chat::Default, "Target is not a client.");
	else {
		c->Message(Chat::Default, "Setting %s's race - zone to take effect",t->GetName());
...
void command_zheader(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] == 0) {
		...
		if (zone->LoadZoneCFG(sep->argplus[1], true))
			c->Message(Chat::Default, "Successfully loaded zone header for %s from database.", sep->argplus[1]);
		else
			c->Message(Chat::Default, "Failed to load zone header %s from database", sep->argplus[1]);
		auto outapp = new EQApplicationPacket(OP_NewZone, sizeof(NewZone_Struct));
		memcpy(outapp->pBuffer, &zone->newzone_data, outapp->size);
		entity_list.QueueClients(c, outapp);
		safe_delete(outapp);
...
void command_zsky(Client *c, const Seperator *sep){
	// modifys and resends zhdr packet
	if (sep->arg[1][0] == 0)
		c->Message(Chat::Default, "Usage: #zsky <sky type>");
	else if (atoi(sep->arg[1])<0 || atoi(sep->arg[1])>255)
		c->Message(Chat::Default, "ERROR: Sky type can not be less than 0 or greater than 255!");
	else {
...
void command_zcolor(Client *c, const Seperator *sep){
	// modifys and resends zhdr packet
	if (sep->arg[3][0] == 0)
		c->Message(Chat::Default, "Usage: #zcolor <red> <green> <blue>");
	else if (atoi(sep->arg[1])<0 || atoi(sep->arg[1])>255)
		c->Message(Chat::Default, "ERROR: Red can not be less than 0 or greater than 255!");
	else if (atoi(sep->arg[2])<0 || atoi(sep->arg[2])>255)
		c->Message(Chat::Default, "ERROR: Green can not be less than 0 or greater than 255!");
	else if (atoi(sep->arg[3])<0 || atoi(sep->arg[3])>255)
		c->Message(Chat::Default, "ERROR: Blue can not be less than 0 or greater than 255!");
	else {
		for (int z = 0; z<4; z++) {
...
void command_gassign(Client *c, const Seperator *sep){
	if (sep->IsNumber(1) && c->GetTarget() && c->GetTarget()->IsNPC() && c->GetTarget()->CastToNPC()->GetSpawnPointID() > 0) {
		...
		c->Message(Chat::Default, "Usage: #gassign [num] - must have an npc target!");
}
...
void command_ai(Client *c, const Seperator *sep)
{
	int arguments = sep->argnum;
	if (!arguments) {
		c->Message(Chat::Default, "Usage: #ai consider [Mob Name] - Show how an NPC considers to a mob");
		c->Message(Chat::Default, "Usage: #ai faction [Faction ID] - Set an NPC's Faction ID");
		c->Message(Chat::Default, "Usage: #ai guard - Save an NPC's guard spot to their current location");
		c->Message(Chat::Default, "Usage: #ai roambox [Min X] [Max X] [Min Y] [Max Y] [Delay] [Minimum Delay] - Set an NPC's roambox using X and Y coordinates");
		c->Message(Chat::Default, "Usage: #ai spells [Spell List ID] - Set an NPC's Spell List ID");
		return;
	}

	if (!c->GetTarget() || !c->GetTarget()->IsNPC()) {
		c->Message(Chat::Default, "You must target an NPC to use this command.");
		return;
	}
	...
	if (is_consider) {
		if (arguments == 2) {
			...
			if (mob_to_consider) {
				...
				c->Message(Chat::Default, fmt::format("Error: {} not found.", mob_name).c_str());
		}
		else {
			c->Message(Chat::Default, "Usage: #ai consider [Mob Name] - Show how an NPC considers a mob");
		}
	}
	else if (is_faction) {
		if (sep->IsNumber(2)) {
		...
		if (target->IsAIControlled()) {
			...
			c->Message(Chat::Default, "You must target an NPC with AI.");
		}
	}
	else if (is_spells) {
		if (sep->IsNumber(2)) {
			...
			if (spell_list_id >= 0) {
				...
				c->Message(Chat::Default, "Spell List ID must be greater than or equal to 0.");
			}
		}
	}
}
...
void command_worldshutdown(Client *c, const Seperator *sep){
	...
	if (worldserver.Connected()) {
		if (
			...
			c->Message(Chat::Default, "World is shutting down now.");
			...
			c->Message(Chat::Default, "World shutdown has been aborted.");
			auto pack = new ServerPacket(ServerOP_ShutdownAll, sizeof(WorldShutDown_Struct));
			WorldShutDown_Struct* wsd = (WorldShutDown_Struct*)pack->pBuffer;
			wsd->time = 0;
			wsd->interval = 0;
			...
			c->Message(Chat::Default, "#worldshutdown - Shuts down the server and all zones.");
			c->Message(Chat::Default, "Usage: #worldshutdown now - Shuts down the server and all zones immediately.");
			c->Message(Chat::Default, "Usage: #worldshutdown disable - Stops the server from a previously scheduled shut down.");
			c->Message(Chat::Default, "Usage: #worldshutdown [timer] [interval] - Shuts down the server and all zones after [timer] seconds and notifies players every [interval] seconds.");
		}
	}
	else
		c->Message(Chat::Default, "Error: World server disconnected");
...
void command_dbspawn2(Client *c, const Seperator *sep){
	...
	if (sep->IsNumber(1) && sep->IsNumber(2) && sep->IsNumber(3)) {
		...
		c->Message(Chat::Default, "Usage: #dbspawn2 spawngroup respawn variance [condition_id] [condition_min]");
	}
}
...
void command_setpass(Client *c, const Seperator *sep){
	int arguments = sep->argnum;
	if (arguments < 2) {
		c->Message(Chat::Default, "Usage: #setpass [Account Name] [Password]");
		return;
	}
...
void command_wp(Client *c, const Seperator *sep){
	...
	if (strcasecmp("add", sep->arg[1]) == 0) {
		...
		c->Message(Chat::Default, "Usage: #wp add/delete grid_num pause wp_num [-h]");
}
...
void command_findspell(Client *c, const Seperator *sep){
	if (sep->arg[1][0] == 0)
		c->Message(Chat::Default, "Usage: #FindSpell [spellname]");
	else if (SPDAT_RECORDS <= 0)
		c->Message(Chat::Default, "Spells not loaded");
	else if (Seperator::IsNumber(sep->argplus[1])) {
		int spellid = atoi(sep->argplus[1]);
		if (spellid <= 0 || spellid >= SPDAT_RECORDS) {
			...
			c->Message(Chat::Default, "  %i: %s", spellid, spells[spellid].name);
		}
	}
	else {
		int count = 0;
		...
		for (int i = 0; i<SPDAT_RECORDS; i++) {
			if (spells[i].name[0] != 0) {
				...
				for (auto & c : sName) c = toupper(c);
				if (sName.find(sCriteria) != std::string::npos && (count <= 20)) {
					c->Message(Chat::Default, "  %i: %s", i, spells[i].name);
					count++;
				}
				else if (count > 20)
					break;
...
void command_setlanguage(Client *c, const Seperator *sep){
	...
	if (!strcasecmp(sep->arg[1], "list")) {
		...
		c->Message(Chat::Default, "Usage: #setlanguage [Language ID] [Language Value]");
		c->Message(Chat::Default, "Usage: #setlanguage [List]");
		c->Message(Chat::Default, "Language ID = 0 to 26", LANG_UNKNOWN2);
		c->Message(Chat::Default, "Language Value = 0 to 100", HARD_SKILL_CAP);
	}
	else {
		LogInfo(
...
void command_setskill(Client *c, const Seperator *sep){
	if (c->GetTarget() == nullptr) {
		c->Message(Chat::Default, "Error: #setskill: No target.");
	}
	else if (!c->GetTarget()->IsClient()) {
	...
	{
		c->Message(Chat::Default, "Usage: #setskill skill x ");
		c->Message(Chat::Default, "       skill = 0 to %d", EQ::skills::HIGHEST_SKILL);
		c->Message(Chat::Default, "       x = 0 to %d", HARD_SKILL_CAP);
	}
	else {
...
void command_race(Client *c, const Seperator *sep){
	...
	if (sep->IsNumber(1) && atoi(sep->arg[1]) >= 0 && atoi(sep->arg[1]) <= 724) {
		...
		c->Message(Chat::Default, "Usage: #race [0-724] (0 for back to normal)");
}
...
void command_gender(Client *c, const Seperator *sep){
	...
	if (sep->IsNumber(1) && atoi(sep->arg[1]) >= 0 && atoi(sep->arg[1]) <= 500) {
		...
		c->Message(Chat::Default, "Usage: #gender [0/1/2]");
}

void command_makepet(Client *c, const Seperator *sep){
	if (sep->arg[1][0] == '\0')
		c->Message(Chat::Default, "Usage: #makepet pet_type_name (will not survive across zones)");
	else
...
void command_texture(Client *c, const Seperator *sep){
	...
	if (sep->IsNumber(1) && atoi(sep->arg[1]) >= 0 && atoi(sep->arg[1]) <= 255) {
		...
		c->Message(Chat::Default, "Usage: #texture [texture] [helmtexture] (0-255, 255 for show equipment)");
}
...
void command_heal(Client *c, const Seperator *sep)
{
	...
	if (c != target) {
		...
		c->Message(Chat::Default, "Healed yourself to full.");
	}
}
...
void command_appearance(Client *c, const Seperator *sep){
	...
	if (sep->arg[2][0] == 0)
		c->Message(Chat::Default, "Usage: #appearance type value");
	else {
		if ((c->GetTarget()))
			t = c->GetTarget();
		t->SendAppearancePacket(atoi(sep->arg[1]), atoi(sep->arg[2]));
		c->Message(Chat::Default, "Sending appearance packet: target=%s, type=%s, value=%s", t->GetName(), sep->arg[1], sep->arg[2]);
	}
}
...
void command_nukeitem(Client *c, const Seperator *sep){
	...
	if (c->GetTarget() && c->GetTarget()->IsClient() && (sep->IsNumber(1) || sep->IsHexNumber(1))) {
		...
		c->Message(Chat::Default, " %u items deleted", numitems);
	}
	else
		c->Message(Chat::Default, "Usage: (targted) #nukeitem itemnum - removes the item from the player's inventory");
}
...
void command_peekinv(Client *c, const Seperator *sep)
{
	...
	if (!c->GetTarget() || !c->GetTarget()->IsClient()) {
		c->Message(Chat::Default, "You must have a PC target selected for this command");
		return;
	}
	...
	if (scopeWhere == 0) {
		c->Message(Chat::Default, "Usage: #peekinv [worn|inv|cursor|trib|bank|trade|world|all]");
		c->Message(Chat::Default, "  Displays a portion of the targeted user's inventory");
		c->Message(Chat::Default, "  Caution: 'all' is a lot of information!");
		return;
	}
	...
	c->Message(Chat::Default, fmt::format("Displaying inventory for {}...", targetClient->GetName()).c_str());
	...
	if (scopeWhere & peekInv)
		c->Message(Chat::Default, fmt::format("Carried Money: {} pp {} gp {} sp {} cp", targetClient->GetPP().platinum, targetClient->GetPP().gold, targetClient->GetPP().silver, targetClient->GetPP().copper).c_str());
	...
	if (scopeWhere & peekCursor) {
		if (targetClient->GetInv().CursorEmpty()) {
			...
			c->Message(Chat::Default, fmt::format("CursorSlot: {}, Item: {} ({}), Charges: {}",
		...
		// money
		c->Message(Chat::Default, fmt::format("Cursor Money: {} pp {} gp {} sp {} cp", targetClient->GetPP().platinum_cursor, targetClient->GetPP().gold_cursor, targetClient->GetPP().silver_cursor, targetClient->GetPP().copper_cursor).c_str());
	}
	...
	if (scopeWhere & peekBank) 
		c->Message(Chat::Default, fmt::format("Bank Money: {} pp {} gp {} sp {} cp", targetClient->GetPP().platinum_bank, targetClient->GetPP().gold_bank, targetClient->GetPP().silver_bank, targetClient->GetPP().copper_bank).c_str());
	...
	if (scopeWhere & peekWorld) {
		...
		if (objectTradeskill == nullptr) {
			c->Message(Chat::Gray, "No world tradeskill object selected...");
		}
		else {
			c->Message(Chat::Default, fmt::format("[WorldObject DBID: {} (entityid: {})]", objectTradeskill->GetDBID(), objectTradeskill->GetID()).c_str());
...
{
	if (sep->arg[1][0] == 0) {
		c->Message(Chat::Default, "Usage: #findnpctype [search criteria]");
		return;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (++count > maxrows) {
			c->Message(Chat::Default, "%i npc types shown. Too many results.", maxrows);
			break;
		}

		c->Message(Chat::Default, "  %s: %s", row[0], row[1]);
	}
...
{
	if (sep->arg[1][0] == 0) {
		c->Message(Chat::Default, "Usage: #findzone [search criteria]");
		return;
	}
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		if (++count > maxrows) {
			c->Message(Chat::Default, "%i zones shown. Too many results.", maxrows);
			break;
		}

		c->Message(Chat::Default, "  %s: %s, %s", row[0], row[1], row[2]);
	}
...
void command_zoneshutdown(Client *c, const Seperator *sep){
	if (!worldserver.Connected())
		...
		c->Message(Chat::Default, "Usage: #zoneshutdown zoneshortname");
	else {
...
void command_attack(Client *c, const Seperator *sep){
	if (c->GetTarget() && c->GetTarget()->IsNPC() && sep->arg[1] != 0) {
		...
		c->Message(Chat::Default, "Usage: (needs NPC targeted) #attack targetname");
}
...
void command_attackentity(Client *c, const Seperator *sep) {
	if (c->GetTarget() && c->GetTarget()->IsNPC() && atoi(sep->arg[1]) > 0) {
		...
		if (sictar)
			...
			c->Message(Chat::Default, "Error: Entity %d not found", atoi(sep->arg[1]));
	}
	else
		c->Message(Chat::Default, "Usage: (needs NPC targeted) #attackentity entityid");
}

void command_serverlock(Client* c, const Seperator* sep)
{
	if (!sep->IsNumber(1)) {
		c->Message(Chat::Default, "Usage: #serverlock [0|1] - Lock or Unlock the World Server (0 = Unlocked, 1 = Locked)");
		return;
	}
...
void command_motd(Client *c, const Seperator *sep)
{
	...
	c->Message(Chat::Default, "MoTD is set.");
	worldserver.SendPacket(pack);
	safe_delete(pack);
}
...
void command_fixmob(Client *c, const Seperator *sep){
	...
	else
	{
		...
		else
		{
			...
			c->Message(Chat::Default, "%s=%i", ChangeType, ChangeSetting);
		}
	}
}
...
void command_gmspeed(Client *c, const Seperator *sep)
{
	...
	if (sep->arg[1][0] != 0) {
		...
		c->Message(Chat::Default, fmt::format("Turning GMSpeed {} for {} (zone to take effect)", state ? "On" : "Off", t->GetName()).c_str());
	}
	else {
		c->Message(Chat::Default, "Usage: #gmspeed [on/off]");
	}
}
...
void command_lastname(Client *c, const Seperator *sep){
	...
	if (strlen(sep->arg[1]) <= 70)
		...
		c->Message(Chat::Default, "Usage: #lastname <lastname> where <lastname> is less than 70 chars long");
}
...
void command_memspell(Client *c, const Seperator *sep){
	...
	{
		c->Message(Chat::Default, "Usage: #MemSpell slotid spellid");
	}
	else
	{
		...
		{
			c->Message(Chat::Default, "Error: #MemSpell: Arguement out of range");
		}
		else
		{
			c->MemSpell(spell_id, slot);
			c->Message(Chat::Default, "Spell slot changed, have fun!");
		}
	}
}
...
void command_bind(Client *c, const Seperator *sep){
	if (c->GetTarget() != 0) {
		if (c->GetTarget()->IsClient())
			...
			c->Message(Chat::Default, "Error: target not a Player");
	}
	else
		c->SetBindPoint();
}
...
void command_repop(Client *c, const Seperator *sep)
{
	...
	if (!force && c->GetTarget() && c->GetTarget()->IsNPC())
	{
		...
		c->Message(Chat::Default, "Repopping %s", c->GetTarget()->GetName());
	}
	else
	{
		int timearg = 1;
		if (force) {
			...
			c->Message(Chat::Default, "Zone depop: Force resetting spawn timers.");
		}

		c->Message(Chat::Default, "Zone depoped. Repoping now.");
		zone->Repop();
		return;
	}
}
...
void command_spawnstatus(Client *c, const Seperator *sep)
{
	...
	{
		c->Message(Chat::Default, "Usage: #spawnstatus <[a]ll (default) | [u]nspawned | [s]pawned | [d]isabled | [e]nabled | {Spawn2 ID}>");
	}
	else {
...
void command_zuwcoords(Client *c, const Seperator *sep){
	// modifys and resends zhdr packet
	if (sep->arg[1][0] == 0)
		c->Message(Chat::Default, "Usage: #zuwcoords <under world coords>");
	else {
...
void command_zsafecoords(Client *c, const Seperator *sep){
	// modifys and resends zhdr packet
	if (sep->arg[3][0] == 0)
		c->Message(Chat::Default, "Usage: #zsafecoords <safe x> <safe y> <safe z>");
	else {
...
void command_freeze(Client *c, const Seperator *sep){
	if (c->GetTarget() != 0)
		...
		c->Message(Chat::Default, "ERROR: Freeze requires a target.");
}
...
void command_unfreeze(Client *c, const Seperator *sep){
	if (c->GetTarget() != 0)
		...
		c->Message(Chat::Default, "ERROR: Unfreeze requires a target.");
}
...
void command_setxp(Client *c, const Seperator *sep){
	...
	if (sep->IsNumber(1)) {
		...
		if (input > 9999999)
			c->Message(Chat::Default, "Error: Value too high.");
		else if(input == -1)
		{
		...
		c->Message(Chat::Default, "Usage: #setxp number or percentage. If 0, will 'rez' a single death. If -1 will subtract a single death.");
}
...
void command_haste(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] != 0) {
		...
		// the primary weapon.
		c->Message(Chat::Default, "Haste set to %d%% - Need to re-equip primary weapon before it takes effect", Haste);
	}
	else
		c->Message(Chat::Default, "Usage: #haste [percentage]");
}

void command_damage(Client *c, const Seperator *sep)
{
	if (c->GetTarget() == 0)
		c->Message(Chat::Default, "Error: #Damage: No Target.");
	else if (!sep->IsNumber(1)) {
		c->Message(Chat::Default, "Usage: #damage [dmg]");
	}
	else {			
		int32 nkdmg = atoi(sep->arg[1]);
		if (nkdmg > 2100000000)
			c->Message(Chat::Default, "Enter a value less then 2,100,000,000.");
...
{
	if (c->GetTarget() == 0)
		c->Message(Chat::Default, "Error: #gmamage: No Target.");
	else if (!sep->IsNumber(1)) {
		c->Message(Chat::Default, "Usage: #gmdamage [dmg] [skipaggro] [spell]");
	}
	else {
		int32 nkdmg = atoi(sep->arg[1]);
...
void command_npcspawn(Client *c, const Seperator *sep){
	int arguments = sep->argnum;
	if (!arguments) {
		c->Message(Chat::Default, "Command Syntax: #npcspawn [Add|Create|Delete|Remove|Update]");
		return;
	}
	...
	if (is_add || is_create) {
		...
		c->Message(Chat::Default, fmt::format("Spawn {} | Name: {} ({})", is_add ? "Added" : "Created", target->GetCleanName(), target->GetID()).c_str());
	}
		...
		c->Message(Chat::Default, fmt::format("Spawn {} | Name: {} ({})", spawn_message, target->GetCleanName(), target->GetID()).c_str());
...
void command_loc(Client *c, const Seperator *sep)
{
	...
	c->Message(Chat::Default, fmt::format(" {} Location | XYZ: {:.2f}, {:.2f}, {:.2f} Heading: {:.2f} ", (c == target ? "Your" : fmt::format(" {} ({}) ", target->GetCleanName(), target->GetID())), target_position.x, target_position.y, target_position.z, (c == target || target->IsClient()) ? target_position.w * 2 : target_position.w).c_str());
	...
	else
	{
		...
		if (newz != BEST_Z_INVALID)
		{
			...
			c->Message(Chat::Default, fmt::format("Best Z is {:.2f} ", newz).c_str());
		}
		else
		{
			c->Message(Chat::Default, "Could not find Best Z.");
		}
	}

	if (!zone->watermap)
	{
		c->Message(Chat::Default, "Water Map not loaded for this zone.");
	}
	else
	{
		...
		switch (region_type) {
			case RegionTypeIce: {
				c->Message(Chat::Default, fmt::format("{} in Ice.", position_string).c_str());
				break;
			}
			case RegionTypeLava: {
				c->Message(Chat::Default, fmt::format("{} in Lava.", position_string).c_str());
				break;
			}
			case RegionTypeNormal: {
				c->Message(Chat::Default, fmt::format("{} in a Normal Region.", position_string).c_str());
				break;
			}
			case RegionTypePVP: {
				c->Message(Chat::Default, fmt::format("{} in a PvP Area.", position_string).c_str());
				break;
			}
			case RegionTypeSlime: {
				c->Message(Chat::Default, fmt::format("{} in Slime.", position_string).c_str());
				break;
			}
			case RegionTypeVWater: {
				c->Message(Chat::Default, fmt::format("{} in VWater (Icy Water?).", position_string).c_str());
				break;
			}
			case RegionTypeWater: {
				c->Message(Chat::Default, fmt::format("{} in Water.", position_string).c_str());
				break;
			}
			default: {
				c->Message(Chat::Default, fmt::format("{} in an Unknown Region.", position_string).c_str());
				break;
			}
		}
	...
	{
		c->Message(Chat::Default, fmt::format(" {} can bind here.", target->GetCleanName()).c_str());
	}
	else if(!target->CanCastBindAffinity() && (zone->IsCity() || zone->IsBindArea(target->GetX(), target->GetY(), target->GetZ())))
	{
		c->Message(Chat::Default, fmt::format(" {} can be bound here.", target->GetCleanName()).c_str());
	}
	else
	{
		c->Message(Chat::Default, fmt::format(" {} cannot bind here.", target->GetCleanName()).c_str());
	}
}
...
void command_goto(Client *c, const Seperator *sep)
{
	...
	if (sep->arg[1][0] == '\0' && c->GetTarget())
		...
		c->Message(Chat::Default, "Usage: #goto [x y z]");
	else
...
void command_flag(Client *c, const Seperator *sep){
	...
	if (!arguments) {
		...
		if (target != c) {
			...
			c->Message(Chat::Default, "Your status level has been refreshed.");
		}
		target->UpdateAdmin();
		return;
	}
	...
	if (
		...
		c->Message(Chat::Default, "Usage: #flag [Status] [Account Name]");
		return;
	}

	auto status = std::stoi(sep->arg[1]);
	if (status < -2 || status > 255) {
		c->Message(Chat::Default, "The lowest a status level can go is -2 and the highest a status level can go is 255.");
		return;
	}
	...
	if (c->Admin() < commandChangeFlags) { //this check makes banning players by less than this level impossible, but i'll leave it in anyways
		c->Message(Chat::Default, "You may only refresh your own flag, doing so now.");
		c->UpdateAdmin();
	}
	else {
		if (status > c->Admin()) {
			...
			c->Message(Chat::Default, "Your status level is not high enough to ban or suspend.");
		}
		else if (!database.SetAccountStatus(account_name, status)) {
			c->Message(Chat::Default, "Failed to set status level.");
		}
		else {
			c->Message(Chat::Default, "Set GM Flag on account.");
...
void command_time(Client *c, const Seperator *sep)
{
	const auto arguments = sep->argnum;
	if (arguments < 1 || !sep->IsNumber(1)) {
		c->Message(Chat::Default, "To set the Time: #time HH [MM]");
		...
		c->Message(Chat::Default, fmt::format("It is now {}.", time_string).c_str());
	...
	c->Message(Chat::Default, fmt::format("Setting world time to {}:{} ...", hours, minutes).c_str());
...
void command_manaburn(Client *c, const Seperator *sep){
	...
	if (c->GetTarget() == 0)
		c->Message(Chat::Default, "#Manaburn needs a target.");
	else {
		int cur_level = c->GetAA(MANA_BURN);//ManaBurn ID
		if (DistanceSquared(c->GetPosition(), target->GetPosition()) > 200)
			c->Message(Chat::Default, "You are too far away from your target.");
		else {
			if (cur_level == 1) {
				if (c->IsAttackAllowed(target))
				{
					...
					if (nukedmg>0)
					{
						...
						c->Message(Chat::Blue, "You unleash an enormous blast of magical energies.");
					}
					Log(Logs::General, Logs::Normal, "Manaburn request from %s, damage: %d", c->GetName(), nukedmg);
				}
			}
			else
				c->Message(Chat::Default, "You have not learned this skill.");
		}
	}
}
...
void command_doanim(Client *c, const Seperator *sep){
	...
	if (!sep->IsNumber(1))
		c->Message(Chat::Default, "Usage: #DoAnim [number]");
	else
		if (c->Admin() >= commandDoAnimOthers)
			if (c->GetTarget() == 0)
				c->Message(Chat::Default, "Error: You need a target.");
			else
...
void command_randomfeatures(Client *c, const Seperator *sep){
	...
	else
	{
		...
		if (Race <= 12 || Race == 128 || Race == 130 || Race == 330) {
			...
			c->Message(Chat::Default, "NPC Features Randomized");
		}
		else
			c->Message(Chat::Default, "This command requires a Playable Race as the Target");
	}
}
...
void command_randtest(Client *c, const Seperator *sep) {
	...
	if (!sep->IsNumber(1)) {
		c->Message(Chat::Default, "Usage: #randtest [iterations]");
		return;
	}
	int total = atoi(sep->arg[1]);
	if (total < 1 || total > 10000000) {
		c->Message(Chat::Default, "Usage: #randtest [iterations] min value 1, max 1000000");
		return;
	}
	int lastval = -1;
	int maxlastval = 0;
	...
	for (int i = 0; i < 100; i++) {
		c->Message(Chat::Default, "Random Results [%i], %i (%.2f %s)", i, results[i], (float)results[i] / (float)total * 100.0f, "%");
	}
	c->Message(Chat::Default, "Same number happened %i times in a row, %i times", maxtimes + 1, maxcount);
}
...
void command_helm(Client *c, const Seperator *sep){
	Mob *target = c->GetTarget();
	if (!sep->IsNumber(1))
		c->Message(Chat::Default, "Usage: #helm [number of helm texture]");
	else if (!target)
		c->Message(Chat::Default, "Error: this command requires a target");
	else {
		uint16 Race = target->GetRace();
		...
		c->Message(Chat::Default, "Helm = %i", atoi(sep->arg[1]));
	}
}
...
void command_hair(Client *c, const Seperator *sep){
	Mob *target = c->GetTarget();
	if (!sep->IsNumber(1))
		c->Message(Chat::Default, "Usage: #hair [number of hair style]");
	else if (!target)
		c->Message(Chat::Default, "Error: this command requires a target");
	else {
		uint16 Race = target->GetRace();
		...
		c->Message(Chat::Default, "Hair = %i", atoi(sep->arg[1]));
	}
}
...
void command_haircolor(Client *c, const Seperator *sep){
	Mob *target = c->GetTarget();
	if (!sep->IsNumber(1))
		c->Message(Chat::Default, "Usage: #haircolor [number of hair color]");
	else if (!target)
		c->Message(Chat::Default, "Error: this command requires a target");
	else {
		uint16 Race = target->GetRace();
		...
		c->Message(Chat::Default, "Hair Color = %i", atoi(sep->arg[1]));
	}
}
...
void command_beard(Client *c, const Seperator *sep){
	Mob *target = c->GetTarget();
	if (!sep->IsNumber(1))
		c->Message(Chat::Default, "Usage: #beard [number of beard style]");
	else if (!target)
		c->Message(Chat::Default, "Error: this command requires a target");
	else {
		uint16 Race = target->GetRace();
		...
		c->Message(Chat::Default, "Beard = %i", atoi(sep->arg[1]));
	}
}
...
void command_beardcolor(Client *c, const Seperator *sep){
	Mob *target = c->GetTarget();
	if (!sep->IsNumber(1))
		c->Message(Chat::Default, "Usage: #beardcolor [number of beard color]");
	else if (!target)
		c->Message(Chat::Default, "Error: this command requires a target");
	else {
		uint16 Race = target->GetRace();
		...
		c->Message(Chat::Default, "Beard Color = %i", atoi(sep->arg[1]));
	}
}
...
void command_scribespells(Client *c, const Seperator *sep) {
	...
	if (sep->argnum < 1 || !sep->IsNumber(1)) {
		c->Message(Chat::Default, "FORMAT: #scribespells <max level> <min level>");
		return;
	}
	...
	if (max_level < 1 || min_level < 1) {
		c->Message(Chat::Default, "ERROR: Level must be greater than or equal to 1.");
		return;
	}

	if (min_level > max_level) {
		c->Message(Chat::Default, "ERROR: Minimum Level must be less than or equal to Maximum Level.");
		return;
	}
...
void command_unscribespell(Client *c, const Seperator *sep){
	...
	if (!sep->arg[1][0]) {
		c->Message(Chat::Default, "FORMAT: #unscribespell <spellid>");
		return;
	}
	...
	if (IsValidSpell(spell_id)) {
		...
		if (book_slot >= 0) {
			...
			t->Message(Chat::Default, "Unscribing spell: %s (%i) from spellbook.", spells[spell_id].name, spell_id);

			if (t != c)
				c->Message(Chat::Default, "Unscribing spell: %s (%i) for %s.", spells[spell_id].name, spell_id, t->GetName());
...
void command_wpinfo(Client *c, const Seperator *sep){
	...
	if (t == nullptr || !t->IsNPC()) {
		c->Message(Chat::Default, "You must target an NPC to use this.");
		return;
	}
...
void command_wpadd(Client *c, const Seperator *sep)
{
	...
	if (t && t->IsNPC())
	{
		...
		{
			c->Message(Chat::Default, "#wpadd FAILED -- Can't determine which spawn record in the database this mob came from!");
			return;
		}
		...
		if (sep->arg[1][0])
		{
			...
			{
				c->Message(Chat::Default, "Usage: #wpadd [pause] [-h]");
				return;
			}
		}
		auto position = c->GetPosition();
		...
		c->Message(Chat::Default, "Waypoint added. Use #wpinfo to see waypoints for this NPC (may need to #repop first).");
	}
	else
		c->Message(Chat::Default, "You must target an NPC to use this.");
}
...
void command_givemoney(Client *c, const Seperator *sep){
	if (!sep->IsNumber(1)) {	//as long as the first one is a number, we'll just let atoi convert the rest to 0 or a number
		...
		c->Message(Chat::Default, "Added %i Platinum, %i Gold, %i Silver, and %i Copper to %s's inventory.", atoi(sep->arg[1]), atoi(sep->arg[2]), atoi(sep->arg[3]), atoi(sep->arg[4]), c->GetTarget()->GetName());
	}
}

void command_itemsearch(Client *c, const Seperator *sep){
	if (sep->arg[1][0] == 0)
		c->Message(Chat::Default, "Usage: #itemsearch [search string]");
	else
	{
		...
		if (Seperator::IsNumber(search_criteria)) {
			...
			if (item) {
				...
				c->Message(Chat::Default, fmt::format(" {} : {} ", item->ID, linker.GenerateLink().c_str()).c_str());

			}
			else {
				c->Message(Chat::Default, fmt::format("Item {} not found", search_criteria).c_str());
			}
			return;
		}
		...
		while ((item = database.IterateItems(&it))) 
		{
			...
			{
				linker.SetItemData(item);
				c->Message(Chat::Default, fmt::format(" {} : {} ", (int)item->ID, linker.GenerateLink().c_str()).c_str());
		...
		if (count == 50)
			c->Message(Chat::Default, "50 items shown...too many results.");
		else
			c->Message(Chat::Default, fmt::format(" {} items found", count).c_str());
	}
}
...
void command_setaaxp(Client *c, const Seperator *sep){
	...
	if (sep->IsNumber(1)) {
		...
		c->Message(Chat::Default, "Usage: #setaaxp <new AA XP value> (<new Group AA XP value> <new Raid XP value>)");
}
...
void command_setaapts(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] == '\0')
		c->Message(Chat::Default, "Usage: #setaapts <new AA points value>");
	else if (atoi(sep->arg[1]) <= 0 || atoi(sep->arg[1]) > 170)
		c->Message(Chat::Default, "You must have a number greater than 0 for points and no more than 170.");
	else {
...
void command_ban(Client *c, const Seperator *sep)
{
	int arguments = sep->argnum;
	if (arguments < 2) {
		c->Message(Chat::Default, "Usage: #ban [Character Name] [Reason]");
		return;
	}
...
void command_oocmute(Client *c, const Seperator *sep){
	if (sep->arg[1][0] == 0 || !(sep->arg[1][0] == '1' || sep->arg[1][0] == '0'))
		c->Message(Chat::Default, "Usage: #oocmute [1/0]");
	else {
...
void command_npcsay(Client *c, const Seperator *sep){
	...
	{
		c->Message(Chat::Default, "Usage: #npcsay message (requires NPC target");
	}
}
...
void command_npcemote(Client *c, const Seperator *sep){
	...
	{
		c->Message(Chat::Default, "Usage: #npcemote message (requires NPC target");
	}
}
...
void command_opcode(Client *c, const Seperator *sep){
	if (!strcasecmp(sep->arg[1], "reload")) {
		ReloadAllPatches();
		c->Message(Chat::Default, "Opcodes for all patches have been reloaded");
	}
}
...
void command_ginfo(Client *c, const Seperator *sep){
	...
	if (!g)
	{
		...
		if (r)
		{
			...
			c->Message(Chat::Default, "Client %s is in a raid.  Raid ID: %u;  Raid leader: %s;  Raid count: %u;  Raid group ID: %u",
			...
			if (t->IsRaidGrouped())
			{
				for (int i = 0; i < MAX_RAID_MEMBERS; i++)
				{
					...
					if (r->members[i].GroupNumber == t_group_id)
					{
						if (rmember)
						{
							c->Message(Chat::Default, "- name: %s (%s)%s%s HP: %u / %u MP: %u / %u", rmember->GetName(), GetClassIDName(rmember->GetClass()),
						...
						{
							c->Message(Chat::Default, "- name: %s (%s)%s%s (not in zone)", r->members[i].membername, GetClassIDName(r->members[i]._class),
								r->members[i].IsGroupLeader ? " (group leader)" : "", r->members[i].IsLooter ? " (looter)" : "");
						}
					}
				}
				...
				c->Message(Chat::Default, "Client is not in a raid group");
		}
		else
			c->Message(Chat::Default, "Client %s is not in a group or raid", t->GetName());

		return;
	}

	c->Message(Chat::Default, "Player %s is in group ID %u with %u members.  Group leader is: %s", t->GetName(), g->GetID(), g->GroupCount(), g->GetLeaderName());
	...
	for (r = 0; r < MAX_GROUP_MEMBERS; r++) {
		if (g->members[r] == nullptr) {
			if (g->membername[r][0] == '\0')
				continue;
			c->Message(Chat::Default, "- Zoned Member: %s", g->membername[r]);
		}
		else {
			c->Message(Chat::Default, "- In-Zone Member: %s (%s) level: %u HP: %u / %u MP: %u / %u", g->membername[r], GetClassIDName(g->members[r]->GetClass()), 
...
void command_aggro(Client *c, const Seperator *sep){
	if (c->GetTarget() == nullptr || !c->GetTarget()->IsNPC()) {
		c->Message(Chat::Default, "Error: you must have an NPC target.");
		return;
	}
...
void command_guildcreate(Client *c, const Seperator *sep){
	...
	{
		c->Message(Chat::Default, "Guild name must be more than 4 characters and less than 16.");
	}
}
...
void command_guildlist(Client *c, const Seperator *sep){
	...
	if (tmp)
	{
		...
		c->Message(Chat::Default, "Could not find reference id.");
}
...
void command_altactivate(Client *c, const Seperator *sep){
	if (sep->arg[1][0] == '\0'){
		c->Message(Chat::Default, "Invalid argument, usage:");
		c->Message(Chat::Default, "#altactivate list - lists the AA ID numbers that are available to you");
		c->Message(Chat::Default, "#altactivate time [argument] - returns the time left until you can use the AA with the ID that matches the argument.");
		c->Message(Chat::Default, "#altactivate [argument] - activates the AA with the ID that matches the argument.");
		return;
	}
	...
	if (!strcasecmp(sep->arg[1], "list")){
		c->Message(Chat::Default, "You have access to the following AA Abilities:");
		int x, val;
		...
		for (x = 0; x < aaHighestID; x++){
			if (AA_Actions[x][0].spell_id || AA_Actions[x][0].action){ //if there's an action or spell associated we assume it's a valid
				...
				if (val){
					...
					c->Message(Chat::Default, "%d: %s %d", x, saa->name, val);
				}
			}
		}
	}
		...
		if (c->GetAA(ability)){
			...
			if (remain)
				c->Message(Chat::Default, "You may use that ability in %d minutes and %d seconds.", (remain / 60), (remain % 60));
			else
				c->Message(Chat::Default, "You may use that ability now.");
		}
		else{
			c->Message(Chat::Default, "You do not have access to that ability.");
		}
	}
	else
	{
...
void command_setgraveyard(Client *c, const Seperator *sep){
	...
	if (!sep->arg[1][0]) {
		c->Message(Chat::Default, "Usage: #setgraveyard [zonename]");
		return;
	}
	...
	if (zoneid > 0) {
		...
		if (graveyard_id > 0) {
			c->Message(Chat::Default, "Successfuly added a new record for this graveyard!");
			if (database.AddGraveyardIDToZone(zoneid, graveyard_id) > 0) {
				c->Message(Chat::Default, "Successfuly added this new graveyard for the zone %s.", sep->arg[1]);
				// TODO: Set graveyard data to the running zone process.
				c->Message(Chat::Default, "Done!");
			}
			else
				c->Message(Chat::Default, "Unable to add this new graveyard to the zone %s.", sep->arg[1]);
		}
		else {
			c->Message(Chat::Default, "Unable to create a new graveyard record in the database.");
		}
	}
	else {
...
void command_advnpcspawn(Client* c, const Seperator* sep)
{
	int arguments = sep->argnum;
	if (!arguments) {
		c->Message(Chat::Default, "Usage: #advnpcspawn addentry [Spawngroup ID] [NPC ID] [Spawn Chance] - Adds a new Spawngroup Entry");
		c->Message(Chat::Default, "Usage: #advnpcspawn addspawn [Spawngroup ID] - Adds a new Spawngroup Entry from an existing Spawngroup");
		c->Message(Chat::Default, "Usage: #advnpcspawn clearbox [Spawngroup ID] - Clears the roambox of a Spawngroup");
		c->Message(Chat::Default, "Usage: #advnpcspawn deletespawn - Deletes a Spawngroup");
		c->Message(Chat::Default, "Usage: #advnpcspawn editbox [Spawngroup ID] [Minimum X] [Maximum X] [Minimum Y] [Maximum Y] [Delay]  - Edit the roambox of a Spawngroup");
		c->Message(Chat::Default, "Usage: #advnpcspawn editrespawn [Respawn Timer] [Variance] - Edit the Respawn Timer of a Spawngroup");
		c->Message(Chat::Default, "Usage: #advnpcspawn makegroup [Spawn Group Name] [Spawn Limit] [Minimum X] [Maximum X] [Minimum Y] [Maximum Y] [Delay] - Makes a new Spawngroup");
		c->Message(Chat::Default, "Usage: #advnpcspawn makenpc - Makes a new NPC");
		c->Message(Chat::Default, "Usage: #advnpcspawn movespawn - Moves a Spawngroup to your current location");
		return;
	}
	...
	if (is_add_entry) {
		if (arguments < 4) {
			c->Message(Chat::Default, "Usage: #advnpcspawn addentry [Spawngroup ID] [NPC ID] [Spawn Chance]");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to add entry to Spawngroup.");
			return;
		}
		...
		if (arguments != 2 || !sep->IsNumber(2)) {
			c->Message(Chat::Default, "Usage: #advnpcspawn clearbox [Spawngroup ID]");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to clear Spawngroup box.");
			return;
		}
		...
		if (!spawn2) {
			c->Message(Chat::Default, "Failed to delete spawn because NPC has no Spawn2.");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to delete spawn.");
			return;
		}
		...
		if (
			...
			c->Message(Chat::Default, "Usage: #advnpcspawn editbox [Spawngroup ID] [Minimum X] [Maximum X] [Minimum Y] [Maximum Y] [Delay]");
			return;
		}
		auto spawngroup_id = std::stoi(sep->arg[2]);
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to edit Spawngroup box.");
			return;
		}
		...
		if (arguments < 2 || !sep->IsNumber(2)) {
			c->Message(Chat::Default, "Usage: #advnpcspawn editrespawn [Respawn Timer] [Variance]");
			return;
		}
		...
		Spawn2* spawn2 = target->respawn2;
		if (!spawn2) {
			c->Message(Chat::Default, "Failed to edit respawn because NPC has no Spawn2.");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to edit respawn.");
			return;
		}
		...
		if (
			...
			c->Message(Chat::Default, "Usage: #advncspawn makegroup [Spawn Group Name] [Spawn Limit] [Minimum X] [Maximum X] [Minimum Y] [Maximum Y] [Delay]");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to make Spawngroup.");
			return;
		}
		...
		Spawn2* spawn2 = target->respawn2;
		if (!spawn2) {
			c->Message(Chat::Default, "Failed to move spawn because NPC has no Spawn2.");
			return;
		}
		...
		auto results = database.QueryDatabase(query);
		if (!results.Success()) {
			c->Message(Chat::Default, "Failed to move spawn.");
			return;
		}
...
void command_aggrozone(Client *c, const Seperator *sep){
	...
	if (!c->GetTarget())
		c->Message(Chat::Default, "Train to you! Last chance to go invulnerable...");
	else
		c->Message(Chat::Default, "Train to %s! Watch them die!!!", c->GetTarget()->GetCleanName());
}
...
void command_modifynpcstat(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] == '\0')
	{
		c->Message(Chat::Default, "usage #modifynpcstat arg value");
		c->Message(Chat::Default, "Args: ac, str, sta, agi, dex, wis, _int, cha, max_hp, mr, fr, cr, pr, dr, runspeed, special_attacks, "
...
void command_globalview(Client *c, const Seperator *sep){
	...
	if (c->GetTarget() && c->GetTarget()->IsNPC())
	{
		...
		c->Message(Chat::Default, "Name, Value");
		while (iter != globalMap.end())
		{
			c->Message(Chat::Default, "%s %s", (*iter).name.c_str(), (*iter).value.c_str());
			++iter;
			++gcount;
		}
		c->Message(Chat::Default, "%u globals loaded.", gcount);
	}
	else
	{
		QGlobalCache *char_c = nullptr;
...
void command_distance(Client *c, const Seperator *sep){
	if (c && c->GetTarget()) {
		...
		c->Message(Chat::Default, "Your target, %s, is %1.1f units from you.", c->GetTarget()->GetName(), Distance(c->GetPosition(), target->GetPosition()));
	}
}
...
void command_starve(Client *c, const Seperator *sep){
	...
	t->SendStaminaUpdate();
	c->Message(Chat::Default, "Target starved.");
...
void command_merchantcloseshop(Client *c, const Seperator *sep){
	Mob *merchant = c->GetTarget();
	if (!merchant || merchant->GetClass() != MERCHANT) {
		c->Message(Chat::Default, "You must target a merchant to close their shop.");
		return;
	}
...
void command_falltest(Client *c, const Seperator *sep)
{
	...
	if(c)
	{
		...
		else
		{
			...
			c->Message(Chat::Default, "Moving to X: %0.2f Y: %0.2f Z: %0.2f", c->GetX(), c->GetY(), zmod);
		}
	}
}
...
void command_push(Client *c, const Seperator *sep) {
	...
	if (!c->GetTarget())
	{
		c->Message(Chat::Default, "You need a target to push.");
		return;
	}
	else if (!c->GetTarget()->IsNPC() && !c->GetTarget()->IsClient())
	{
		c->Message(Chat::Default, "That's an invalid target, nerd.");
		return;
	}
	else if (!sep->IsNumber(1))
	{
	...
	else
	{
		...
		if (success)
		{
			c->Message(Chat::Default, "%s was pushed for %0.1f!", t->GetCleanName(), pushback);
			return;
		}
		else
		{
			c->Message(Chat::Default, "Pushed failed on %s. Coord check likely failed.", t->GetCleanName());
			return;
		}
	}
}
...
void command_logs(Client *c, const Seperator *sep){
	...
	if (sep->argnum > 0) {
		...
		if (strcasecmp(sep->arg[1], "list_settings") == 0 || (strcasecmp(sep->arg[1], "set") == 0 && strcasecmp(sep->arg[3], "") == 0)){
			c->Message(Chat::Default, "[Category ID | console | file | gmsay | Category Description]");
			...
			for (int i = 0; i < Logs::LogCategory::MaxCategoryID; i++){
				...
				c->Message(Chat::Default, StringFormat("--- %i | %u | %u | %u | %s", i, LogSys.log_settings[i].log_to_console, LogSys.log_settings[i].log_to_file, LogSys.log_settings[i].log_to_gmsay, Logs::LogCategoryName[i]).c_str());
				redisplay_columns++;
			}
		}
		/* #logs set */
		if (strcasecmp(sep->arg[1], "set") == 0){
			if (strcasecmp(sep->arg[2], "console") == 0){
				...
				c->Message(Chat::Default, "--- #logs set [console|file|gmsay] <category_id> <debug_level (1-3)> - Sets log settings during the lifetime of the zone");
				c->Message(Chat::Default, "--- #logs set gmsay 20 1 - Would output Quest errors to gmsay");
			}
			if (logs_set == 1){
		...
		if (strcasecmp(sep->arg[1], "quiet") == 0)
		{
			...
			c->Message(Chat::Yellow, "Shhh. Be vewy vewy quiet, I'm hunting wabbits.");
		}
	}
	else 
	{
		c->Message(Chat::Default, "#logs usage:");
		c->Message(Chat::Default, "--- #logs quiet - Turns off all gmsay logs in the current zone until the next time the zone resets.");
		c->Message(Chat::Default, "--- #logs reload_all - Reload all settings in world and all zone processes with what is defined in the database");
		c->Message(Chat::Default, "--- #logs list_settings - Shows current log settings and categories loaded into the current process' memory");
		c->Message(Chat::Default, "--- #logs set [console|file|gmsay] <category_id> <debug_level (1-3)> - Sets log settings during the lifetime of the zone");
	}
}
...
void command_setgreed(Client *c, const Seperator *sep)
{
	if (!sep->IsNumber(1))
	{
		c->Message(Chat::Default, "Usage: #setgreed [greed]");
		return;
	}
	...
	if (c->GetTarget() && c->GetTarget()->IsNPC())
	{
		...
		c->Message(Chat::Default, "Set %s greed to %d (%d percent)", c->GetTarget()->GetName(), value, c->GetTarget()->CastToNPC()->GetGreedPercent());
	}
	else
	{
		c->Message(Chat::Default, "Please select a NPC target to set greed.");
		return;
	}
...
void command_godmode(Client *c, const Seperator *sep){
	...
	if (sep->arg[1][0] != 0)
	{
		...
		c->Message(Chat::Default, "Turning GodMode %s for %s (zone for gmspeed to take effect)", state ? "On" : "Off", c->GetName());
	}
	else
		c->Message(Chat::Default, "Usage: #godmode [on/off]");
}
...
void command_cleartimers(Client *c, const Seperator *sep)
{
	...
	if(sep->IsNumber(1))
	{
		type = atoi(sep->arg[1]);
		t->ClearPTimers(type);
		c->Message(Chat::Default, "Cleared PTimer %d on %s", type, t->GetName());
	}
	else
	{
		int x = 0;
		if (strcasecmp(sep->arg[1], "help") == 0)
		{
			c->Message(Chat::Default, "PTimer list: (1 Unused) (2 Surname) (3 FD) (4 Sneak) (5 Hide) (6 Taunt) (7 InstallDoubt)");
			c->Message(Chat::Default, "(8 Fishing) (9 Foraging) (10 Mend) (11 Tracking) (12 SenseTraps) (13 DisarmTraps)");
			c->Message(Chat::Default, "(14-16 Discs) (17 Combat Ability) (18 Begging/PP) (19 Sense Heading) (20 Bind Wound)");
			c->Message(Chat::Default, "(21 Apply Posion) (22 Disarm) (23 PEQZone)");
			c->Message(Chat::Default, "(1000-2999 AAs) (3001-4999 AA Effects) (5000-9678 Spells)");
			c->Message(Chat::Default, "(5087 Lay Hands) (5088 Harm Touch)");
		}
		else if(strcasecmp(sep->arg[1], "all") == 0)
		{
			t->ClearPTimers(0);
			t->SendAATimers();
			t->ResetAllSkills();
			c->Message(Chat::Default, "Cleared all timers on %s", t->GetName());
		}
		else if(strcasecmp(sep->arg[1], "skills") == 0)
		{
			t->ResetAllSkills();
			c->Message(Chat::Default, "Cleared all skill timers on %s", t->GetName());
		}
		else if(strcasecmp(sep->arg[1], "disc") == 0)
		{
			...
			c->Message(Chat::Default, "Cleared all disc timers on %s", t->GetName());
		}
		else if(strcasecmp(sep->arg[1], "aa") == 0)
		{
			...
			t->SendAATimers();
			c->Message(Chat::Default, "Cleared all AA timers on %s", t->GetName());
		}
		else if(strcasecmp(sep->arg[1], "spells") == 0)
		{
			...
			c->Message(Chat::Default, "Cleared all spell timers on %s", t->GetName());
		}
		else
		{
			c->Message(Chat::Default, "Usage: #cleartimers [type]/help/all/skills/disc/aa/spells");
		}
...
void command_damagetotals(Client *c, const Seperator *sep)
{
	...
	if (!target || !target->IsNPC())
	{
		c->Message(Chat::Default, "Please target a NPC to use this command on.");
	}
	else
	{
...
void command_showhelm(Client *c, const Seperator *sep)
{
	...
	if (sep->arg[1][0] != 0) 
	{
		...
		{
			c->Message(Chat::Default, "There was no change in your showhelm setting.");
			return;
		}
		...
		c->Message(Chat::Default, "Usage: #showhelm on/off [all]");
